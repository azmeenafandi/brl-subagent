...
- Delivery mode by status: `failed`/`stopped` → steering you to act; `completed` → follow-up. (The `completionNotify` knob controls whether an idle conductor is triggered — see below.)
- Knob dependency: polling is correct only when wakes are disabled (`completionNotify: "off"`). Under `"all"` or `"failed"`, a terminal run always wakes you, so polling is wrong. One status check as a stall check is legitimate; repeated polling is not.
